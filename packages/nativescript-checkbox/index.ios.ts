...
import { CheckBoxInterface } from './common';
...
const boxTypeProperty = new Property<CheckBox, BEMBoxType>({
	name: 'boxType',
	valueConverter: (v) => {
		return BoxType[v] === BoxType.circle ? BEMBoxType.Circle : BEMBoxType.Square;
	},
});
...
export class CheckBox extends Button implements CheckBoxInterface {
	...
	constructor() {
		...
		this._iosCheckbox = BEMCheckBox.alloc().initWithFrame(CGRectMake(0, 0, 80, 80)) as BEMCheckBox;
	}
	...
	set onAnimationType(value: number) {
		if (this._iosCheckbox) this._iosCheckbox.onAnimationType = this._getAnimationType(value);
